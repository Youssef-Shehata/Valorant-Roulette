...
const MAP_BASED = {
  ...
  lotus: {
    ...
    sentinels: ["veto", "chamber", "cypher", "killjoy", "sage", "deadlock"],
  ...
  fracture: {
    ...
    sentinels: ["veto", "chamber", "cypher", "killjoy", "sage"],
    duelists: [
      "jett",
      "neon",
      "phoenix",
...
const generateTeam = (agentData) => {
  ...
  const duelists = AGENT_DATA.duelists.filter(
    (a) => a !== roles.find((r) => r.role === "duelists")?.agent,
  );
...
function App() {
  usePrefetchImages(Object.values(AGENT_ASSETS));
      ...
      <main className="main-content">
        {view === "menu" && (
          <div className="role-selection">
            <div className="role-card " onClick={() => setView("team")}>
              <h2>Random Team</h2>
            ...
            <div className="role-card " onClick={() => setView("map")}>
              <h2>Map Based</h2>
        ...
        {view == "map" && (
          <div className={`results-container ${view}`}>
            ...
            <div style={{ paddingTop: "2rem", fontSize: "20px" }}>
              Generated teams are based on the owner's taste, and follow no
              industry standards ( I don't watch VCT ).
            </div>
          </div>
        )}
        {view !== "menu" && view != "map" && (
          <div className={`results-container ${view}`}>
            <button className="close-button" onClick={() => setView("menu")}>
              &times;
            </button>
